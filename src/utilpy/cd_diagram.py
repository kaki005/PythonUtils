...
def graph_ranks(
    ...
    **kwargs,
    ...
    def hfl(lis):
        return [a * hf for a in lis]

    def wfl(lis):
        return [a * wf for a in lis]
    ...
    def line(lis, color="k", **kwargs):
        """
        Input is a list of pairs of points.
        """
        ax.plot(wfl(nth(lis, 0)), hfl(nth(lis, 1)), color=color, **kwargs)
    ...
    def draw_lines(lines, side=0.05, height=0.1):
        ...
        for left, right in lines:
            line(
                [(rankpos(ssums[left]) - side, start), (rankpos(ssums[right]) + side, start)], linewidth=linewidth_sign
            )
            start += height
            print("drawing: ", left, right)
...
def draw_cd_diagram(df_perf: pd.DataFrame, alpha=0.05, title=None, labels=False):
    """
...
def wilcoxon_holm(df_perf: pd.DataFrame, alpha=0.05):
    """
