...
class GridSearch:
    ...
    def fit(self, estimator, variables: dict) -> None:
        """
    ...
    def __train_and_evaluate(self, estimator, params: tuple, variables: dict) -> None:
        """
        ...
        variables : (dict)
            Variables to be used in the create_variables method.
        """
        params_dict = dict(zip(self.keys, params, strict=False))
    ...
    def __update_score_grids(self, score: float, params_dict: dict) -> None:
        ...
        for combination in itertools.combinations(self.keys, 2):
            self.score_grids[combination][
                self.param_grid[combination[0]].index(params_dict[combination[0]]),
                self.param_grid[combination[1]].index(params_dict[combination[1]]),
            ] = score
    ...
    def __plot_heatmaps(self) -> None:
        ...
        num_cols = num_combinations // num_rows + (num_combinations % num_rows > 0)
